...
(sudo apt-get update &&
	...
	curl -L https://go.dev/dl/go1.25.7.linux-amd64.tar.gz | sudo tar -xz -C /usr/local/)
...
elif [[ "$go_version" =~ "go version go1.25.7" ]]; then
	echo "go check: go will use system go ($go_version)"
else
	echo "go check: system go must either be 1.25.7 or not installed"
	exit 1
fi
java_version=`java -version 2>&1`
if [[ ! "$java_version" =~ 'openjdk version "21.0.8"' ]]; then
	if [[ ! "$java_version" =~ 'openjdk version "21.0.9"' ]]; then
	    echo "java check: 21.0.8 or 21.0.9 required ($java_version)"
	    exit 1
	fi
fi
