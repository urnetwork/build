...
(sudo apt-get update &&
	...
	curl -L https://go.dev/dl/go1.26.1.linux-amd64.tar.gz | sudo tar -xz -C /usr/local/)
...
elif [[ "$go_version" =~ "go version go1.26.1" ]]; then
	echo "go check: go will use system go ($go_version)"
else
	echo "go check: system go must either be 1.26.1 or not installed"
	exit 1
fi
